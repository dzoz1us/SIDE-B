...
export default function RecordsPage() {
  const { navigate, records, branches } = useApp();
  ...
  const labels = ["all", ...Array.from(new Set(records.map(r => r.label).filter(Boolean)))];

  const filtered = useMemo(() => {
    let result = records.filter(r => {
      const matchSearch = r.title.toLowerCase().includes(search.toLowerCase()) ||
        r.ensemble_name.toLowerCase().includes(search.toLowerCase());
      const matchLabel = labelFilter === "all" || r.label === labelFilter;
      const matchStock = stockFilter === "all" || (stockFilter === "instock" && r.stock_quantity > 0);
      const matchBranch = branchFilter === "all" || r.branch_name === branches.find(b => b.id === Number(branchFilter))?.name;
      return matchSearch && matchLabel && matchStock && matchBranch;
    });
    if (sort === "price-asc") result = [...result].sort((a, b) => Number(a.retail_price) - Number(b.retail_price));
    if (sort === "price-desc") result = [...result].sort((a, b) => Number(b.retail_price) - Number(a.retail_price));
    if (sort === "year") result = [...result].sort((a, b) => new Date(b.release_date).getTime() - new Date(a.release_date).getTime());
    if (sort === "popularity") result = [...result].sort((a, b) => b.sold_current_year - a.sold_current_year);
    return result;
  }, [records, search, labelFilter, stockFilter, branchFilter, sort, branches]);
    ...
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10">
      ...
      <FilterBar>
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input type="text" placeholder="Поиск по названию или ансамблю..." value={search}
          ...
          {paged.map(record => (
            <div key={record.id} className="group bg-card border border-border overflow-hidden flex flex-col">
              ...
              <div className="p-3 flex flex-col flex-1">
                <h4 className="font-medium text-foreground text-xs leading-snug line-clamp-2 mb-1">{record.title}</h4>
                <p className="text-muted-foreground text-xs mb-0.5">{record.ensemble_name}</p>
                <p className="text-muted-foreground text-xs mb-3">{record.release_date?.slice(0, 4)} · {record.label}</p>
                <div className="flex items-center justify-between mt-auto">
                  <span className="text-primary font-medium text-sm">{formatPrice(Number(record.retail_price))}</span>
                </div>
                <Btn variant="outline" onClick={() => navigate("record", { id: record.id })} className="w-full mt-2 justify-center text-xs py-1.5">Подробнее</Btn>
              </div>
            </div>
          ))}
        </div>
