...
import { PageTitle, FormInput, SelectInput, Pagination, FilterBar } from "../../components/ui";
...
export default function EnsemblesPage() {
  ...
  const types = ["all", ...Array.from(new Set(ensembles.map(e => e.type)))];
  const countries = ["all", ...Array.from(new Set(ensembles.map(e => e.country)))];
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "albums") result = result.sort((a, b) => b.albumCount - a.albumCount);
    return result;
    ...
    <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10">
      ...
      <FilterBar>
        ...
        <SelectInput
          ...
          options={types.map(t => ({ value: t, label: t === "all" ? "Все типы" : t }))}
        ...
        <SelectInput
          ...
          options={[
            ...
            { value: "albums", label: "По кол-ву альбомов" },
          ]}
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-4 text-xs text-muted-foreground border-t border-border/50 pt-3">
                  <span className="flex items-center gap-1.5">
                    <Disc size={12} className="text-primary/60" /> {ensemble.albumCount} альбомов
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Users size={12} className="text-primary/60" /> {ensemble.memberCount} участников
                  </span>
                </div>
              </div>
            </button>
